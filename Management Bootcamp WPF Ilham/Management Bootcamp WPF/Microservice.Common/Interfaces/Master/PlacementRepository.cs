...
namespace Microservice.Common.Interfaces.Master
{
    public class PlacementRepository : IPlacementRepository
    {
        ...
        public List<Placement> Search(string keywoard, string category)
        {
            ...
            {
                return _context.Placements.Where(x => (x.IsDelete == false) && (x.Address.Contains(keywoard))).ToList();
            }
            else if (category == "Kelurahan")
            {
                return _context.Placements.Where(x => (x.IsDelete == false) && (x.Kelurahan.Contains(keywoard))).ToList();
            }
            else if (category == "Kecamatan")
            {
                return _context.Placements.Where(x => (x.IsDelete == false) && (x.Kecamatan.Contains(keywoard))).ToList();
            }
            else if (category == "Kabupaten")
            {
                return _context.Placements.Where(x => (x.IsDelete == false) && (x.Kabupaten.Contains(keywoard))).ToList();
            }
            else if (category == "Phone")
            {
                return _context.Placements.Where(x => (x.IsDelete == false) && (x.Phone.Contains(keywoard))).ToList();
            }
            else
            {
                return _context.Placements.Where(x => x.IsDelete == false).ToList();
